...
def extract_dino_feature_2d(patch_2d, model, transform):
    ...
    input_tensor = transform(img_3ch).unsqueeze(0)  
    with torch.no_grad():
...
def compute_spatial_features(supervoxels):
    ...
    for label in labels:
        ...
        if is_3d:
            feature_dict["centroid_x"] = centroid[2]
            feature_dict["size_x"] = size[2]
        else:
            feature_dict["centroid_x"] = 0
            feature_dict["size_x"] = 0
            
        spatial_feats.append(feature_dict)
        
    return pd.DataFrame(spatial_feats)

def get_adjacency(supervoxels):
    structure = ndimage.generate_binary_structure(supervoxels.ndim, 1)
